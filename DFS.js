...
async function Dfs(map,final){
    ...
    while(lista.length>0){
        ...
        if(!(analisar.getCordX() == final.x && analisar.getCordY() == final.y)) {
            ...
            if(aux.moverPraCima()) {
                    //aux.showEstado();
                lista.unshift(new PathFinderAgent(cloneMatrix(aux.getEstado()),(aux.getEstadoDaTentativa()+cont), analisar,aux.getCordX(),aux.getCordY()));
                cont++;
            }
